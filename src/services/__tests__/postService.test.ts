...
describe('PostService', () => {
  ...
  describe('searchPosts', () => {
    it('should search posts by caption', async () => {
      const searchPromise = postService.searchPosts('test');
      jest.advanceTimersByTime(100);
    ...
    it('should return empty array for non-matching query', async () => {
      const searchPromise = postService.searchPosts('nonexistentquery12345');
      jest.advanceTimersByTime(100);
      const results = await searchPromise;
    ...
    it('should perform case-insensitive search', async () => {
      const searchPromise1 = postService.searchPosts('test');
      jest.advanceTimersByTime(100);
      const results1 = await searchPromise1;

      const searchPromise2 = postService.searchPosts('TEST');
      jest.advanceTimersByTime(100);
    ...
    it('should search by username', async () => {
      const searchPromise = postService.searchPosts('user');
      jest.advanceTimersByTime(100);
      const results = await searchPromise;
    ...
    it('should handle empty query', async () => {
      const searchPromise = postService.searchPosts('');
      jest.advanceTimersByTime(100);
      const results = await searchPromise;
